...
class BalanceSheetAll extends Component
{
    ...
    private function generateReport()
    {
        ...
        // Total bank or hand balance up until the selected month
        ...
        $setsPurchased = Stock::where('date', '<=', $endDate)
                              ->sum('sets');
                            //   dd( $setsPurchased );

        // Total sets sold (branch + head office) during the selected month
        $setsSoldBranch = BranchSale::where('date', '<=', $endDate)
                                    ->sum('sets');
                                    // dd($setsSoldBranch);

        $setsSoldHeadOffice = HeadOfficeSale::where('date', '<=', $endDate)
                                            ->sum('sets');
        // dd($setsSoldHeadOffice);

        // Total rejected or free sets during the selected month
        $setsRejectedOrFree = RejectOrFree::where('date', '<=', $endDate)
                                          ->sum('sets');

                                        //   dd($totalStampAvailable);
        ...
        // Average stamp price per set (this remains unchanged)
        $totalSetsBuy = Stock::where('date', '<=', $endDate)->sum('sets');
        $totalSetsBuyPrice = Stock::where('date', '<=', $endDate)->sum('total_price');
        $averageStampPricePerSet = $totalSetsBuy > 0 ? $totalSetsBuyPrice / $totalSetsBuy : 0;

        // dd($totalStockEndOfMonth);
    ...
    private function calculateTotalBankOrHandBalance($endDate)
    {
        ...
        // Calculating the total balance up until the selected month
        $totalBalance = $cashInTotal + $totalSalesReceipts - $cashOutTotal - $totalPurchases - $totalExpenses;
    ...
    public function downloadPdf()
    {
        $data = [
            ...
            'month' => Carbon::create($this->selectedYear, $this->selectedMonth)->format('F'),
